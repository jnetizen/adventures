#!/usr/bin/env bash
#
# Upload Grizzy & Lemmings adventure assets to Supabase Storage.
#
# Usage:
#   export SUPABASE_URL=https://<project-ref>.supabase.co
#   export SUPABASE_SERVICE_KEY=<your-service-role-key>
#   ./scripts/upload-grizzy-lemmings.sh
#
set -euo pipefail
...
FAMILY="jenny-family"
ADVENTURE="grizzy-lemmings-heist"
PUBLIC_DIR="public"
...
# Scene images
for f in "$PUBLIC_DIR"/images/scenes/grizzy-lemmings/*.png; do
  [ -f "$f" ] && upload_file "$f" "$FAMILY/$ADVENTURE/$(basename "$f")"
done

# Cutscene images
for f in "$PUBLIC_DIR"/images/cutscenes/grizzy-lemmings/*.png; do
  [ -f "$f" ] && upload_file "$f" "$FAMILY/$ADVENTURE/$(basename "$f")"
done

# Climax video
if [ -f "$PUBLIC_DIR/videos/grizzy-lemmings/climax.mp4" ]; then
  upload_file "$PUBLIC_DIR/videos/grizzy-lemmings/climax.mp4" "$FAMILY/$ADVENTURE/climax.mp4"
fi
...
echo ""
echo "==> Test URL:"
echo "    $SUPABASE_URL/storage/v1/object/public/$BUCKET/$FAMILY/$ADVENTURE/prologue.png"
